...
it.each(
  Object.values(NodeType).filter((nodeType) => nodeType !== NodeType.Text),
)("should show the icon for type %s", (nodeType) => {
  ...
  expect(
    screen.getByTestId(`icon: ${ICON_BY_NODE_TYPE[props.type]()}`),
  ).toBeVisible();
});
...
it("should show the FilePdf icon when NodeType is Text and mimeType is application/pdf", () => {
  ...
  expect(
    screen.getByTestId(
      `icon: ${ICON_BY_NODE_TYPE[props.type](MIME_TYPE["application/pdf"])}`,
    ),
  ).toBeVisible();
});
...
it("should show the FileText icon when NodeType is Text and mimeType is not application/pdf ", () => {
  ...
  expect(
    screen.getByTestId(
      `icon: ${ICON_BY_NODE_TYPE[props.type](MIME_TYPE["text/plain"])}`,
    ),
  ).toBeVisible();
});
...
it.each(
  Object.values(NodeType).filter((nodeType) => nodeType !== NodeType.Text),
)("should show the colored icon for type %s", (nodeType) => {
  const props = listItemPropsBuilder({ type: nodeType });
  render(
	<ThemeProvider>
	<ListItem {...props} />
  ...
  expect(
    screen.getByTestId(`icon: ${ICON_BY_NODE_TYPE[props.type]()}`),
  ).toHaveStyle({ color: ICON_COLOR_BY_NODE_TYPE[props.type]() });
});
